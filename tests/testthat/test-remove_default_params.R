...
test_that("remove_default_params() removes params that match geom defaults", {
  ...
  expect_identical(params_list_nodefaults,
                   list(arrow = arrow(30L,
                                      unit(0.1,
                                           "inches"),
                                      "last",
                                      "closed")))
...
test_that("remove_default_params() leaves intact params that aren't in defaults", {
  ...
  expect_identical(remove_default_params("geom_text", params_list),
                   params_list)
})
